...
public class AQJmsSubscriber extends AQApplication {
	...
	/**
	 * The subscription id is assigned according command line arguments, set to 0 if
	 * not specified
	 */
	...
	protected void doTest(JMSTopic aq) throws JMSException, SQLException {
		...
		TopicReceiver subscriber = ((AQjmsSession) aq.session).createTopicReceiver(topic, // Used topic
																							// Name and identification
																							// of the subscription
				"SUBSCRIPTION" + _subscriptionId, null); // No message selector

		// Receive message
		System.out.println(
				"Waiting 60 seconds to receive message for subscription SUBSCRIPTION" + _subscriptionId + " ...");
		ObjectMessage objectMessage = (ObjectMessage) subscriber.receive(60000); // Wait in milliseconds
