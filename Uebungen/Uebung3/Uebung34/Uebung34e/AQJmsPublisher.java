...
public class AQJmsPublisher extends AQApplication {
	...
	protected void doTest(JMSTopic aq) throws JMSException, SQLException {
		...
		TopicPublisher publisher = aq.session.createPublisher(null); // No specific topic here
																		// Different ones can be used with same
																		// publisher
		// Get topic object
		Topic topic = ((AQjmsSession) aq.session).getTopic(DB_AQ_ADMIN_NAME, // Queue owner
				"MULTI_QUEUE"); // Queue name

		// Prepare messages
		messageData = new AQJmsMultiQueueItem(0, "Published message", "Message has been published to subscriber 0");
		ObjectMessage objectMessage0 = aq.session.createObjectMessage(messageData);

		messageData = new AQJmsMultiQueueItem(1, "Published message", "Message has been published to subscriber 1");
		...
		messageData = new AQJmsMultiQueueItem(3, "Published message",
				"Message has been published to subscribers 1 and 2");
		...
		recipientList0[0] = new AQjmsAgent("SUBSCRIPTION0", // Name and identification of the subscription
